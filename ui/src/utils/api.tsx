...
export const fetchBatchDeleteTools = async (ids: number[]) => {
    const { data } = await axios.post(`/api/admin/tools/batch-delete`, ids);
    return data?.data || {};
};
...
export const fetchToolsPage = async (page: number, size: number, q?: string, catelog?: string) => {
    ...
    const { data } = await axios.get(`/api/admin/tools?${params.toString()}`);
    return data?.data || {};
};